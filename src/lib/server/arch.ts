...
const packageMetadataSchema = v.strictObject({
  name: v.string(), fullVersion: v.string(), architecture: v.picklist(['x86_64', 'aarch64']),
  installedSize: v.pipe(v.number(), v.integer(), v.minValue(0)),
  depends: v.pipe(v.array(v.string()), v.maxLength(256)),
  provides: v.pipe(v.array(v.string()), v.maxLength(256)),
  conflicts: v.pipe(v.array(v.string()), v.maxLength(256)),
  replaces: v.pipe(v.array(v.string()), v.maxLength(256)),
});
...
function dependencyOperator(value: string | undefined): ArchDependency['operator'] {
  switch (value) {
    case '>=':
    case '<=':
    case '=':
...
export function parseArchDependency(value: string): ArchDependency | null {
  ...
  const operator = dependencyOperator(match[2]);
...
function relationList(value: string[], provides: boolean, allowSoname: boolean): string[] | null {
  const result: string[] = [];

  for (const item of value) {
    if (item.length === 0 || item.length > 256 || item.includes('\u0000') || item.includes('\u001f') || item.includes('\u007f') || /\s/.test(item)) return null;
...
export function parsePackageMetadata(value: unknown): PackageMetadata | null {
  const parsed = v.safeParse(packageMetadataSchema, value);

  if (!parsed.success) return null;
  const object = parsed.output;

  if (!ARCH_PACKAGE_NAME.test(object.name) || !parseArchPackageVersion(object.fullVersion)) return null;
