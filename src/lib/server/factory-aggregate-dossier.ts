...
async function cohortDossier(env: Env, targetId: string, runId?: string): Promise<FactoryAggregateDossier> {
  ...
  const revisionIds = [...new Set(members.map((member) => member.recipe_revision_id).filter((id): id is string => Boolean(id)))];
  const buildsByRevision = new Map<string, Array<{ id: string; attempt: number; status: string; architecture: Architecture; artifact_filename: string | null; artifact_sha256: string | null; artifact_size: number | null; created_at: number }>>();
  const buildIds: string[] = [];
  for (let offset = 0; offset < revisionIds.length; offset += 256) {
    const batch = revisionIds.slice(offset, offset + 256); const placeholders = batch.map(() => '?').join(',');
    const builds = await query<{ id: string; revision_id: string; attempt: number; status: string; architecture: Architecture; artifact_filename: string | null; artifact_sha256: string | null; artifact_size: number | null; created_at: number }>(env.DB,
      `SELECT id,revision_id,attempt,status,architecture,artifact_filename,artifact_sha256,artifact_size,created_at FROM builds WHERE revision_id IN (${placeholders}) ORDER BY revision_id,architecture`, ...batch);
    for (const build of builds) { const list = buildsByRevision.get(build.revision_id) ?? []; list.push(build); buildsByRevision.set(build.revision_id, list); buildIds.push(build.id); latest = Math.max(latest, build.created_at); }
  }
  const artifactsByBuild = new Map<string, Array<{ filename: string; sha256: string; size: number }>>();
  for (let offset = 0; offset < buildIds.length; offset += 256) {
    const batch = buildIds.slice(offset, offset + 256); const placeholders = batch.map(() => '?').join(',');
    const artifacts = await query<{ build_id: string; filename: string; sha256: string; size: number }>(env.DB,
      `SELECT build_id,filename,sha256,size FROM build_artifacts WHERE build_id IN (${placeholders}) ORDER BY build_id,filename`, ...batch);
    for (const artifact of artifacts) { const list = artifactsByBuild.get(artifact.build_id) ?? []; list.push(artifact); artifactsByBuild.set(artifact.build_id, list); }
  }
  const dossierByRevision = new Map<string, string>();
  for (let offset = 0; offset < revisionIds.length; offset += 256) {
    const batch = revisionIds.slice(offset, offset + 256); const placeholders = batch.map(() => '?').join(',');
    const dossiers = await query<{ id: string; revision_id: string }>(env.DB, `SELECT id,revision_id FROM factory_dossiers WHERE revision_id IN (${placeholders}) ORDER BY revision_id,created_at DESC,rowid DESC`, ...batch);
    for (const dossier of dossiers) if (!dossierByRevision.has(dossier.revision_id)) dossierByRevision.set(dossier.revision_id, dossier.id);
  }
  for (const member of members) {
    ...
    const builds = buildsByRevision.get(member.recipe_revision_id) ?? [];
    const dossierId = dossierByRevision.get(member.recipe_revision_id) ?? null;
    const retained = builds.flatMap((build) => (artifactsByBuild.get(build.id) ?? []).map((artifact) => ({ ...artifact, architecture: build.architecture })));
...
async function imageDossier(env: Env, targetId: string, runId?: string): Promise<FactoryAggregateDossier> {
  ...
  if (packageRevisionIds.size) {
    ...
    const buildsByRevision = new Map<string, Array<{ id: string; attempt: number; status: string; architecture: Architecture; artifact_filename: string | null; artifact_sha256: string | null; artifact_size: number | null }>>();
    const buildIds: string[] = [];
    for (let offset = 0; offset < revisions.length; offset += 256) {
      const batch = revisions.slice(offset, offset + 256).map((revision) => revision.id); const marks = batch.map(() => '?').join(',');
      const builds = await query<{ id: string; revision_id: string; attempt: number; status: string; architecture: Architecture; artifact_filename: string | null; artifact_sha256: string | null; artifact_size: number | null }>(env.DB,
        `SELECT id,revision_id,attempt,status,architecture,artifact_filename,artifact_sha256,artifact_size FROM builds WHERE revision_id IN (${marks}) ORDER BY revision_id,architecture`, ...batch);
      for (const build of builds) { const list = buildsByRevision.get(build.revision_id) ?? []; list.push(build); buildsByRevision.set(build.revision_id, list); buildIds.push(build.id); }
    }
    const artifactsByBuild = new Map<string, Array<{ filename: string; sha256: string; size: number }>>();
    for (let offset = 0; offset < buildIds.length; offset += 256) {
      const batch = buildIds.slice(offset, offset + 256); const marks = batch.map(() => '?').join(',');
      const artifacts = await query<{ build_id: string; filename: string; sha256: string; size: number }>(env.DB, `SELECT build_id,filename,sha256,size FROM build_artifacts WHERE build_id IN (${marks}) ORDER BY build_id,filename`, ...batch);
      for (const artifact of artifacts) { const list = artifactsByBuild.get(artifact.build_id) ?? []; list.push(artifact); artifactsByBuild.set(artifact.build_id, list); }
    }
    const dossierByRevision = new Map<string, string>();
    for (let offset = 0; offset < revisions.length; offset += 256) {
      const batch = revisions.slice(offset, offset + 256).map((revision) => revision.id); const marks = batch.map(() => '?').join(',');
      const dossiers = await query<{ id: string; revision_id: string }>(env.DB, `SELECT id,revision_id FROM factory_dossiers WHERE revision_id IN (${marks}) ORDER BY revision_id,created_at DESC,rowid DESC`, ...batch);
      for (const dossier of dossiers) if (!dossierByRevision.has(dossier.revision_id)) dossierByRevision.set(dossier.revision_id, dossier.id);
    }
    for (const revision of revisions) {
      const dossierId = dossierByRevision.get(revision.id) ?? null;
      const builds = buildsByRevision.get(revision.id) ?? [];
      const retained = builds.flatMap((build) => (artifactsByBuild.get(build.id) ?? []).map((artifact) => ({ ...artifact, architecture: build.architecture })));
      ...
      constituents.push({ key: `package:${revision.id}`, pkgbase: revision.name, revisionId: revision.id, dossierId, status: statusForBuilds(builds, dossierId), architectures: [...new Set(builds.map((build) => build.architecture))].sort(), outputs, error: builds.find((build) => build.status === 'failed') ? 'Constituent package build failed; inspect its dossier.' : null });
    }
  }
...
export async function buildFactoryAggregateDossier(env: Env, input: { targetKind: AggregateDossierKind; targetId: string; runId?: string }): Promise<FactoryAggregateDossier> {
  ...
  const dossier = await (input.targetKind === 'cohort' ? cohortDossier(env, input.targetId, input.runId) : imageDossier(env, input.targetId, input.runId));
  dossier.id = `aggregate-${(await sha256(canonicalJson(dossier))).slice(0, 48)}`;
  return dossier;
}
...
export async function createFactoryAggregateDossier(env: Env, actor: string, input: { targetKind: AggregateDossierKind; targetId: string; runId?: string }): Promise<StoredFactoryAggregateDossier> {
  const dossier = await buildFactoryAggregateDossier(env, input);
  ...
  try {
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO factory_aggregate_dossiers(id,target_kind,target_id,run_id,canonical_json,canonical_sha256,markdown,markdown_sha256,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)`)
        .bind(dossier.id, input.targetKind, input.targetId, dossier.identity.runId, canonical, canonicalSha256, markdown, markdownSha256, actor, now()),
    ...
    const existing = await storedFactoryAggregateDossier(env, dossier.id);
    if (existing.canonicalSha256 !== canonicalSha256 || existing.markdownSha256 !== markdownSha256) throw new PolicyError(409, 'Aggregate dossier ID is already bound to different evidence.');
    return existing;
  }
