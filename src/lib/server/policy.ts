...
export function parseDeclaredLicense(input: unknown): string {
  ...
  if (!value || value.length > 256 || hasControlCharacters(value)) return invalidDeclaredLicense();
...
export function revisionImage(revision: Pick<Revision, 'build_images_json' | 'image_digest'>, architecture: Architecture): string {
  if (!isArchitecture(architecture)) throw new PolicyError(409, 'Select supported build architectures.');
  ...
  if (entries.some(([name, image]) => !isArchitecture(name) || typeof image !== 'string' || !fullPinnedImage.test(image))) {
...
export async function validateRevision(revision: Revision) {
  ...
  if ((publicRecipe === null) !== (publicRecipeSha256 === null) ||
      (publicRecipe !== null && await sha256(publicRecipe) !== publicRecipeSha256)) {
  ...
  if (revision.description !== undefined && revision.description !== null &&
      (!revision.description.trim() || revision.description.length > 160 || revision.description.includes('\u0000') || revision.description.includes('\r') || revision.description.includes('\n'))) {
