...
export function normalizeCandidate(candidate: FactoryCandidate): FactoryCandidate {
  ...
  if (recipeMode === 'template') {
    if (!candidate.template || candidate.buildCommands.length || candidate.packageCommands.length || candidate.smokeCommands.length ||
      (candidate.vendorArtifact && !['deb-v1', 'rpm-v1', 'appimage-v1', 'run-v1'].includes(candidate.template.id))) {
      throw new Error('Template mode requires empty command arrays and only verified vendor preparation');
    }

    const requiresVerifiedArtifact = ['deb-v1', 'rpm-v1', 'appimage-v1', 'run-v1'].includes(candidate.template.id);

    if (requiresVerifiedArtifact !== Boolean(candidate.vendorArtifact)) throw new Error('Verified vendor template requires an inspected artifact manifest');

    if (candidate.vendorArtifact) {
      const expected = candidate.template.id === 'deb-v1' ? ['deb'] : candidate.template.id === 'rpm-v1' ? ['rpm'] : candidate.template.id === 'run-v1' ? ['run'] : ['appimage1', 'appimage2'];

      if (!expected.includes(candidate.vendorArtifact.format)) throw new Error('Vendor artifact format does not match typed template');
    }
  ...
  if ((candidate.template?.id === 'go-v1' || candidate.template?.id === 'go-v2') && !makeDependencies.some((dependency) => parseArchDependency(dependency)?.name === 'go')) {
    throw new Error(`${candidate.template.id} requires go in makeDependencies`);
  }
...
export async function assembleRecipeRevision(candidate: FactoryCandidate, recipe: string, lint: RecipeLint, revisionId?: string): Promise<FactoryRevisionDraft> {
  ...
  const recipePolicy = candidate.template ? {
    ...
    ...(candidate.vendorArtifact ? { vendorArtifact: candidate.vendorArtifact } : {}),
...
export async function persistFactoryRevision(
  ...
  actor = 'factory',
  generationId?: string,
  options: { privateCandidate?: boolean } = {},
  ...
  const privateCandidate = options.privateCandidate === true;
  if (generationId === undefined || request.factory_run_id !== generationId || (!privateCandidate && request.status !== 'generating') || (privateCandidate && !['generating', 'review'].includes(request.status))) {
  ...
  const origin = revision.preserved_origin_revision_id ?? null;
  const insert = origin
    ? env.DB.prepare(`INSERT INTO revisions (
      ...
      .bind(
        ...
        revision.commit_sha, revision.created_at, origin,
      )
    : env.DB.prepare(`INSERT INTO revisions (
      id, request_id, version, recipe, recipe_sha256, public_recipe, public_recipe_sha256, manifest_sha256, sources_json,
      dependencies_json, make_dependencies_json, smoke_commands_json, architectures_json, build_images_json, pkgrel, source_date_epoch,
  ...
  statements.push(
    insert,
    audit(env.DB, actor, privateCandidate ? 'factory_candidate_revision_created' : 'factory_revision_created', revision.id, {
