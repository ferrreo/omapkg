...
function text(value: unknown, label: string, maximum = 256): string {
  if (typeof value !== 'string' || !value || value.length > maximum || /[\u0000-\u001f\u007f]/.test(value)) reject(`${label} is invalid.`, 400);
  return value;
}

function digest(value: unknown, label: string): string {
  if (typeof value !== 'string' || !SHA256.test(value)) reject(`${label} is invalid.`, 400);
  return value;
}
...
function collection(value: unknown): Collection {
  if (!collections.includes(value as Collection)) reject('Owned repository collection is invalid.', 400);
...
function decode(value: string): string | null {
  try { const decoded = decodeURIComponent(value); return decoded && decoded !== '.' && decoded !== '..' && !/[\u0000\r\n]/.test(decoded) ? decoded : null; } catch { return null; }
}
