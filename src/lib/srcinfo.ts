...
export function parseSrcinfo(text: string): Srcinfo {
  if (new TextEncoder().encode(text).length > 1024 * 1024 || /[\x00\r]/.test(text)) throw new Error('Invalid or oversized .SRCINFO');
  let pkgbase = '';
  ...
  for (const line of text.split('\n')) {
    ...
    if (!match || match[2].length > 4096 || /[\x00-\x1f\x7f]/.test(match[2])) throw new Error('Malformed .SRCINFO field');
