...
test('v2 qualification reads registered output rows when aggregate build artifact fields are null', async () => {
  ...
  const artifact = 'b'.repeat(64); const provenance = JSON.stringify({ outputs: [{ pkgbase: 'demo', filename: 'demo-1.0-1-x86_64.pkg.tar.zst', artifactSha256: artifact,
...
test('qualification ABI index ingests and then reuses real retained ABI chunks', async () => {
  ...
  const artifactSha256 = 'a'.repeat(64); const chunk: AbiChunk = { schemaVersion: 1, kind: 'abi-records', artifactSha256, start: 0, records: [
