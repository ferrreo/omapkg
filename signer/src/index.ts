import { assertOutputEvidence, assertStandaloneReproducibilityContract, requireReproducibilityContract } from '../../src/lib/server/output-evidence';
...
function assertIntentShape(intent: SignIntent, now: number, maxBytes: number, identity: Identity): void {
  ...
  if (build.privateCandidate === true) throw new Error('private factory candidates cannot be signed');
  ...
  if (build.factoryRunId && (provenance.factoryRunId !== build.factoryRunId || provenance.factoryAttempt !== build.factoryAttempt || provenance.factoryInputSha256 !== build.factoryInputSha256)) {
    throw new Error('factory execution identity does not match reviewed build');
  }
...
async function verifyAttestation(intent: SignIntent): Promise<void> {
  ...
  if (provenance.schemaVersion === 2) {
    ...
    await requireReproducibilityContract(report);
  ...
  } else if (intent.kind !== 'database') {
    if (intent.build.surface === 'binary') await assertStandaloneReproducibilityContract(provenance.reproducibility, {
      architecture: intent.build.architecture as 'x86_64' | 'aarch64', recipeSha256: provenance.recipeSha256, imageDigest: provenance.imageDigest,
      sourceDateEpoch: provenance.sourceDateEpoch, sources: provenance.sources, inputLockSha256: '', dependencyPlan: provenance.dependencyPlan,
      ...(intent.kind === 'package' ? { output: { filename: intent.artifact.filename, size: intent.artifact.size, sha256: intent.artifact.sha256 } } : {}),
    });
