...
tool_inventory=$("$runtime" run --rm --network none "$manifest_ref" bash -lc 'set -e; for package in buildah crun mtools e2fsprogs util-linux qemu-img gptfdisk dosfstools jq grub; do archive=$(find /var/cache/pacman/pkg -maxdepth 1 -type f \( -name "$package-*.pkg.tar.zst" -o -name "$package-*.pkg.tar.xz" \) -print -quit); if [[ -z "$archive" ]]; then echo "missing package archive for $package" >&2; exit 1; fi; version=$(pacman -Q "$package"); archive_version=$(pacman -Qp "$archive"); [[ "$archive_version" == "$version" ]] || { echo "package archive does not match installed $package: $archive_version != $version" >&2; exit 1; }; sha=$(sha256sum "$archive" | cut -d" " -f1); jq -c -n --arg package "$package" --arg version "$version" --arg archive "$(basename "$archive")" --arg sha "$sha" "{package:\$package,version:\$version,archive:\$archive,sha256:\$sha}"; done' | jq -s .)
