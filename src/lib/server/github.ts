...
type RequiredRepository = { owner: string; name: string };

function base64Url(value: string | Uint8Array): string {
  return Buffer.from(value).toString('base64url');
}

function requiredRepository(env: GitHubEnv): RequiredRepository {
...
async function installationToken(env: GitHubEnv): Promise<string> {
  ...
  const body = v.safeParse(v.object({ token: v.pipe(v.string(), v.minLength(1)) }), await response.json());

  if (!body.success) throw new Error('GitHub App returned no installation token.');
