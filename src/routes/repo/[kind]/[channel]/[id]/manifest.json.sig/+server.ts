...
export const GET: RequestHandler = async ({ platform, params }) => {
  ...
  if (!resolved) error(404, 'Manifest signature not found.');
  const response = await distributionManifestObject(platform.env, params.id ?? '', resolved.manifestKind, true, resolved.channel);
  if (!response) error(404, 'Manifest signature not found.');
  return response;
};
