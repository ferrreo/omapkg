      ...
      db.prepare(`INSERT INTO worker_uploads
        ...
        WHERE b.id = ? AND b.worker_id = ? AND b.lease_token = ? AND b.status = 'leased' AND b.lease_expires_at > ?
          ...
          AND (b.private_candidate=1 AND EXISTS (SELECT 1 FROM factory_runs fr JOIN factory_run_attempts fa ON fa.run_id=fr.id AND fa.attempt=fr.current_attempt
            ...
            OR b.private_candidate=0 AND EXISTS (SELECT 1 FROM approvals a WHERE a.revision_id = b.revision_id AND a.kind = 'area' AND a.manifest_sha256 = r.manifest_sha256 AND a.revoked_at IS NULL)
      ...
      db.prepare(`INSERT OR IGNORE INTO worker_upload_parts(upload_id, part_number, sha256, size, etag, created_at)
        SELECT ?, ?, ?, ?, ?, ? WHERE EXISTS (
          ...
          WHERE u.id = ? AND u.build_id = ? AND u.worker_id = ? AND u.status = 'active' AND u.attempt = ? AND u.lease_token = ?
            ...
            AND (b.private_candidate=1 AND EXISTS (SELECT 1 FROM factory_runs fr JOIN factory_run_attempts fa ON fa.run_id=fr.id AND fa.attempt=fr.current_attempt
              WHERE fr.id=b.factory_run_id AND fr.status='running' AND fr.current_attempt=b.factory_attempt AND fr.lease_expires_at>unixepoch() AND fa.status='running' AND fa.lease_expires_at>unixepoch() AND fa.candidate_revision_id=b.revision_id)
              OR b.private_candidate=0 AND EXISTS (SELECT 1 FROM approvals a WHERE a.revision_id = b.revision_id AND a.kind = 'area' AND a.manifest_sha256 = (SELECT manifest_sha256 FROM revisions WHERE id = b.revision_id) AND a.revoked_at IS NULL)
      ...
      storedOutputContract(build)
        ...
        WHERE id = ? AND worker_id = ? AND lease_token = ? AND status = 'leased' AND lease_expires_at > ? AND artifact_key IS NULL
          ...
          AND (builds.private_candidate=1 AND EXISTS (SELECT 1 FROM factory_runs fr JOIN factory_run_attempts fa ON fa.run_id=fr.id AND fa.attempt=fr.current_attempt
            WHERE fr.id=builds.factory_run_id AND fr.status='running' AND fr.current_attempt=builds.factory_attempt AND fr.lease_expires_at>unixepoch() AND fa.status='running' AND fa.lease_expires_at>unixepoch() AND fa.candidate_revision_id=builds.revision_id)
            OR builds.private_candidate=0 AND EXISTS (SELECT 1 FROM approvals a WHERE a.revision_id = builds.revision_id AND a.kind = 'area' AND a.manifest_sha256 = (SELECT manifest_sha256 FROM revisions WHERE id = builds.revision_id) AND a.revoked_at IS NULL)
