...
export function parseDependencyBlockers(value: unknown): DependencyBlockerInput[] {
  ...
  if (!Array.isArray(value) || !value.length || value.length > 16) throw new Error('Report between 1 and 16 dependency blockers');
  return value.map((item) => {
    if (!item || typeof item !== 'object' || Object.keys(item).some((key) => !['relation', 'phase', 'resolution', 'detail', 'findingSha256'].includes(key)) ||
        !['factory', 'build', 'runtime'].includes(item.phase) || !['dependency', 'recipe', 'exception'].includes(item.resolution) ||
        typeof item.detail !== 'string' || !item.detail.trim() || item.detail.length > 4096 || /[\u0000\r]/.test(item.detail) ||
        (item.relation != null && (typeof item.relation !== 'string' || item.relation.length > 256 || !parseArchRelation(item.relation))) ||
        (item.resolution === 'dependency' && !item.relation) ||
        (item.findingSha256 !== undefined && !/^[a-f0-9]{64}$/.test(item.findingSha256))) {
      throw new Error('Invalid dependency blocker; report evidence without executable input or admission authority');
    }
    return { relation: item.relation ?? null, phase: item.phase, resolution: item.resolution, detail: item.detail,
      ...(item.findingSha256 === undefined ? {} : { findingSha256: item.findingSha256 }) };
  });
}
...
export async function checkDependencyLinks(db: D1Database, links: Array<{ parent: string; child: string; blockerId: string }>): Promise<number> {
  ...
  const memo = new Map<string, { depth: number; nodes: Set<string> }>();
  const visit = (node: string, path: Set<string>): { depth: number; nodes: Set<string> } => {
...
export async function resolveDependencyBlockers(env: Pick<Env, 'DB'>, requestId?: string): Promise<void> {
  ...
  for (const blocker of blockers) {
    ...
    const provider = releases.find((release) => {
      ...
      try { const metadata = parsePackageMetadata(JSON.parse(release.provenance).packageMetadata); return metadata && satisfiesArchRelation(relation, metadata); }
      catch { return false; }
    });
