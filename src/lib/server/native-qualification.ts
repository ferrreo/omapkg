...
export const reproducibilityStatuses = ['verified-reproducible', 'mismatch', 'not-checked'] as const;
...
function parseProfile(value: unknown): { id: string; sha256: string } {
  ...
  const idValue = text(item.id, 'profile id', 128); if (!identifier.test(idValue)) throw new PolicyError(400, 'Invalid profile id.');
...
export async function parseQualificationPlan(value: unknown): Promise<QualificationPlanPayload> {
  ...
  const commands = item.commands; if (!Array.isArray(commands) || commands.length < 1 || commands.length > 32) throw new PolicyError(400, 'Qualification plan needs bounded commands.');
  const observations = item.observations; if (!Array.isArray(observations) || observations.length < 1 || observations.length > 32) throw new PolicyError(400, 'Qualification plan needs bounded observations.');
...
export async function reviewQualificationPlan(env: Env, actor: Actor | null, planId: string, kind: 'area' | 'security', reason: string) {
  humanMaintainer(actor); if (!['area', 'security'].includes(kind)) throw new PolicyError(400, 'Choose an area or security review.');
  ...
  const plan = await currentPlan(env.DB, planId); if (!plan) throw new PolicyError(404, 'Qualification plan not found.');
...
function parseResult(value: unknown): QualificationEvidence['result'] {
  ...
  const commands = item.commands.map((value) => { const command = record(value, 'qualification command result'); exactKeys(command, ['name', 'exitCode', 'passed', 'stdoutSha256', 'stderrSha256'], 'qualification command result');
    const name = text(command.name, 'result command name', 128); if (!identifier.test(name)) throw new PolicyError(400, 'Invalid result command name.');
...
function parseEvidence(value: unknown): QualificationEvidence {
  ...
  const environmentDetails = record(environment.details, 'environment details'); if (Object.keys(environmentDetails).length > 128) throw new PolicyError(400, 'Environment details are too large.');
  ...
  const workerId = text(item.workerId, 'worker id', 128); if (!identifier.test(workerId)) throw new PolicyError(400, 'Invalid worker id.');
...
function parseReproducibility(value: unknown): ReproducibilityReport {
  ...
  const parseAttempt = (value: unknown): ReproducibilityAttempt => { const attempt = record(value, 'reproducibility attempt'); exactKeys(attempt, ['buildId', 'attempt', 'workerId', 'artifactSha256', 'outputs'], 'reproducibility attempt');
    ...
    const outputs = attempt.outputs.map((value) => { const output = record(value, 'reproducibility output'); exactKeys(output, ['filename', 'sha256'], 'reproducibility output'); return { filename: text(output.filename, 'output filename', 256), sha256: hash(output.sha256, 'output digest') }; }).sort((a, b) => a.filename.localeCompare(b.filename));
...
function signedPayload(evidence: QualificationEvidence): Omit<QualificationEvidence, 'signature'> {
  const { signature: _signature, ...payload } = evidence; return payload;
}
...
async function actualReproducibility(env: Env, plan: QualificationPlanPayload, report: ReproducibilityReport): Promise<ReproducibilityStatus> {
  ...
  if (rows[0].input_lock_sha256 !== plan.inputSha256 || contracts.some((contract) => {
    const cohort = contract.cohort; return !cohort || typeof cohort !== 'object' || Array.isArray(cohort) || (cohort as Record<string, unknown>).id !== plan.cohortId || (cohort as Record<string, unknown>).revision !== plan.revision || (cohort as Record<string, unknown>).manifestSha256 !== plan.candidateSha256;
  ...
  if (report.status === 'verified-reproducible' && !equal) throw new PolicyError(409, 'Reproducibility report claims equal bytes for mismatched artifacts.');
...
async function verifyRetainedSignature(env: Env, key: string, digestValue: string): Promise<void> {
  ...
  const object = await env.ARTIFACTS.get(key); if (!object || object.size <= 0 || object.size > 16 * 1024) throw new PolicyError(409, 'Retained native signature object is unavailable.');
  const bytes = new Uint8Array(await object.arrayBuffer()); if (bytes.byteLength !== object.size || await sha256(bytes) !== digestValue) throw new PolicyError(409, 'Retained native signature bytes changed.');
}
...
export async function recordNativeQualification(env: Env, auth: AuthenticatedWorker, value: unknown): Promise<{ id: string; duplicate: boolean; status: QualificationStatus; reproducibilityStatus: ReproducibilityStatus | null; reportSha256: string }> {
  ...
  if (evidence.operation === 'reproducibility') {
    ...
    status = reproducibilityStatus === 'verified-reproducible' ? 'passed' : reproducibilityStatus === 'mismatch' ? 'failed' : 'not-checked';
...
export async function addQualificationException(env: Env, actor: Actor | null, input: { evidenceId: string; subjectSha256: string; reason: string; expiresAt: number }) {
  ...
  if (evidence.status === 'passed' || evidence.reproducibility_status === 'verified-reproducible') throw new PolicyError(409, 'A passing qualification does not need an exception.');
