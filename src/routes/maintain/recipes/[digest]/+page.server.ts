...
export const load: PageServerLoad = async (event) => {
 ...
 try { await inputAuthority(env.DB, actor); canInspect = true; } catch { /* Other maintainers can inspect retained evidence. */ }
 ...
 if (!text.includes('\0')) preview = text; } catch { /* Binary data is available as a download. */ }
  }
