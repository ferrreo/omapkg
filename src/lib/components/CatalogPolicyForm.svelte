...
<form method="POST" {action} class="form-grid" use:enhance={() => { busy = true; return async ({ update }) => { await update(); busy = false; }; }}>
  ...
  <div class="field field--full"><label for="catalog-portable">Portable outputs within a native build</label><input id="catalog-portable" name="portableOutputs" value={value?.portableOutputs?.join(' ') ?? ''} /><p class="field__hint">Optional space-separated names, such as split documentation or data packages. These use any; remaining outputs use the native target.</p></div>
  <div class="field field--full"><label for="catalog-runtime-groups">Packages installed together for native tests</label><textarea id="catalog-runtime-groups" name="runtimeGroups" rows="3">{value?.runtimeGroups?.map((group) => group.join(' ')).join('\n') ?? ''}</textarea><p class="field__hint">Leave empty to test all outputs together. For conflicting variants, use one group per line with space-separated names. Every output must appear; shared dependencies can appear in several groups.</p></div>
