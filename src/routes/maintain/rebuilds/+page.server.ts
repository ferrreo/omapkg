...
export const actions: Actions = {
  preview: (event) => { maintainer(event);

 return formAction(event, async (form) => ({ preview: await previewRebuildCohort(environment(event).DB, field(form, 'cohortId'), report(form)) })); },
  create: (event) => { maintainer(event);

 return formAction(event, async (form) => {
