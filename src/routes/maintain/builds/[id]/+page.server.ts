...
export const load: PageServerLoad = async (event) => {
 ...
 try { humanMaintainer(actor, request?.area); canSign = !!request && build.status === 'succeeded'; } catch { /* Read access remains available. */ }

  let canRetain = false;

 try { await inputAuthority(DB, actor); canRetain = !!build.input_lock_sha256 && canSign; } catch { /* Retention requires system authority. */ }
