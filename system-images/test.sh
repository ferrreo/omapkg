...
printf '%s\n' '#!/usr/bin/env bash' 'set -euo pipefail' 'for ((index=1; index<=$#; index++)); do' '  if [[ "${!index}" == --output ]]; then next=$((index + 1)); output=${!next}; fi' 'done' 'mkdir -p "$output"' 'cp "$FIXTURE_SEED"/* "$output/"' >"$fixture/fake-client"
chmod +x "$fixture/fake-client"
FIXTURE_SEED="$fixture/seed" "$root/scripts/build-system-image.sh" --check --allow-http --manifest http://example/tx --key "$fixture/trusted-key.asc" --fingerprint "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" --client "$fixture/fake-client" --profile "$root/system-images/profiles/x86_64-uefi.json" | grep -F 'verified image lock:' >/dev/null
printf '%s' '{"kind":"system","lane":"system","tampered":true}' >"$fixture/seed/system.json"
set +e
FIXTURE_SEED="$fixture/seed" "$root/scripts/build-system-image.sh" --check --allow-http --manifest http://example/tx --key "$fixture/trusted-key.asc" --fingerprint "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" --client "$fixture/fake-client" --profile "$root/system-images/profiles/x86_64-uefi.json" >"$fixture/failure.out" 2>&1
status=$?
set -e
(( status != 0 ))
