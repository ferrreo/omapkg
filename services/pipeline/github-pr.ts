...
type RecipeFileDraft = { manifest: Record<string, unknown> & { packageName: string };
  revision: Pick<Revision, 'id' | 'sbom_json' | 'architectures_json' | 'public_recipe' | 'recipe' | 'lint_json' | 'preserved_origin_revision_id'> };
...
export async function revisionRecipeFiles(env: FactoryEnv, draft: RecipeFileDraft): Promise<RecipeTreeFile[]> {
  ...
  const capturedRecipe = new TextDecoder('utf-8', { fatal: true }).decode(inspected.get('PKGBUILD'));
  const repaired = Boolean(draft.revision.preserved_origin_revision_id);

  if (!repaired && capturedRecipe !== draft.revision.recipe) throw new Error('Preserved PKGBUILD differs from reviewed bytes');
  ...
  for (const file of capture.manifest.files) {
    ...
    const bytes = file.path === 'PKGBUILD' && repaired ? new TextEncoder().encode(draft.revision.recipe) : await recipeCaptureBytes(env, file.object);
    originals.push({ path: `${path}/${file.path}`, bytes, mode: file.mode });
  }
