...
export async function claimJob(
  ...
  if (candidate.revision_surface === 'binary' && !capabilities.includes('single-build-reproducibility-v1')) return null;
    ...
    await db.batch([
      db.prepare(`UPDATE builds AS b SET status = 'leased', worker_id = ?, lease_token = ?, lease_expires_at = ?,
        ...
        WHERE id = ? AND architecture = ?
          ...
          AND (b.private_candidate=1 AND EXISTS (SELECT 1 FROM factory_runs fr JOIN factory_run_attempts fa ON fa.run_id=fr.id AND fa.attempt=fr.current_attempt
            WHERE fr.id=b.factory_run_id AND fr.status='running' AND fr.current_attempt=b.factory_attempt AND fr.lease_expires_at>unixepoch() AND fa.status='running' AND fa.lease_expires_at>unixepoch() AND fa.candidate_revision_id=b.revision_id)
            OR (b.private_candidate=0 AND EXISTS (SELECT 1 FROM approvals a WHERE a.revision_id = b.revision_id AND a.kind = 'area' AND a.manifest_sha256 = ? AND a.revoked_at IS NULL)
      ...
      db.prepare(`UPDATE requests SET status='building',updated_at=? WHERE id=? AND status='queued' AND ?=0
        ...
        .bind(timestamp, candidate.revision_request_id, Number(candidate.private_candidate ?? 0), candidate.revision_id, candidate.id, candidate.revision_id, worker.id, leaseToken),
      db.prepare(`INSERT INTO audit_events(actor,action,target,detail,created_at)
  ...
  return {
    ...
    ...(claimed.private_candidate === 1 ? { privateCandidate: true, factoryRunId: claimed.factory_run_id ?? undefined, factoryAttempt: claimed.factory_attempt ?? undefined, factoryInputSha256: claimed.factory_input_sha256 ?? undefined } : {})
  };
}
