...
export async function reconcileCatalogImports(db: D1Database, actor: Actor | null, candidateId: string, baselineId: string) {
  ...
  const sourceWarnings = [...candidate.manifest.sources, ...baseline.manifest.sources].flatMap((source) =>
    source.status === 'captured' && source.signature !== 'verified'
      ? [`${source.id}: source signature ${source.signature}; metadata equality is not trust approval`]
      : []);
