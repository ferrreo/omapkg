...
type AuditExportDetail = {
  format: AuditExportFormat; range: AuditRange; from: number | null; to: number | null; limit: number;
  snapshotMaxId: number; filtered: boolean; requestId: string | null; returned?: number; pages?: number;
};
...
export function parseAuditQuery(source: URL | URLSearchParams, maxLimit = DEFAULT_LIMIT): AuditQuery {
  ...
  if (q.length > MAX_QUERY_LENGTH || [...q].some((character) => character <= '\u001f' || character === '\u007f')) return invalidQuery('Audit search must be 256 characters or fewer.');
...
function exportDetail(options: AuditQuery, format: AuditExportFormat, snapshotMaxId: number, extra: Pick<AuditExportDetail, 'returned' | 'pages'> = {}): AuditExportDetail {
  return {
    format,
    range: options.range,
    from: options.from,
