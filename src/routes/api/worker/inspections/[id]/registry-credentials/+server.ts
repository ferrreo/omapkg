...
import { issueScopedRegistryCredentials } from '$lib/server/worker-registry';
...
export const POST: RequestHandler = async (event) => {
  try {
    ...
    return json(await issueScopedRegistryCredentials(env, auth.worker, job.id, job.image_ref), { headers: { 'Cache-Control': 'no-store', Pragma: 'no-cache', 'Referrer-Policy': 'no-referrer' } });
