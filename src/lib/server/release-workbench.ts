...
function statusFor(row: CandidateRow, value: ReleaseManifest): ReleaseStatus {
  ...
  return row.status === 'active' ? value.channel === 'stable' ? 'stable' : 'testing'
    : row.status === 'held' ? 'blocked' : row.status === 'signed' ? 'testing' : 'candidate';
}
...
function toView(row: CandidateRow, value: ReleaseManifest, historical: CandidateRow[], approvals: ApprovalRow[], pointers: PointerRow[], selectedArchitecture: Architecture | null): ReleaseView {
  ...
  const history = historical.filter((item) => item.kind === row.kind && item.release_id === row.release_id).map((item) => ({
  ...
  return {
    ...
    summary: value.kind === 'system' ? `Omarchy ${value.identity.version ?? 'system'} system manifest with ${value.packageCount} package${value.packageCount === 1 ? '' : 's'}.`
      : value.kind === 'opr' ? `Independent OPR generation ${value.identity.generation ?? 'not recorded'} with ${value.packageCount} package${value.packageCount === 1 ? '' : 's'}.`
        : `Resolved transaction for system ${value.identity.version ?? 'not recorded'} and OPR ${value.identity.generation ?? 'not recorded'}.`,
