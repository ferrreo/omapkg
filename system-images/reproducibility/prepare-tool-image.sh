...
RUN pacman-key --init \
    ...
    && pacman --config /tmp/opr-pacman.conf -Sy --noconfirm \
      $buildah_package crun mtools e2fsprogs util-linux qemu-img gptfdisk dosfstools jq grub archlinux-keyring \
...
tool_inventory=$("$runtime" run --rm --network none "$manifest_ref" bash -lc 'set -e; for package in buildah crun mtools e2fsprogs util-linux qemu-img gptfdisk dosfstools jq grub archlinux-keyring; do version=$(pacman -Q "$package"); archive=; while IFS= read -r candidate; do [[ "$(pacman -Qp "$candidate")" == "$version" ]] || continue; archive=$candidate; break; done < <(find /var/cache/pacman/pkg -maxdepth 1 -type f \( -name "$package-*.pkg.tar.zst" -o -name "$package-*.pkg.tar.xz" \) -print); if [[ -z "$archive" ]]; then echo "missing matching package archive for $version" >&2; exit 1; fi; sha=$(sha256sum "$archive" | cut -d" " -f1); jq -c -n --arg package "$package" --arg version "$version" --arg archive "$(basename "$archive")" --arg sha "$sha" "{package:\$package,version:\$version,archive:\$archive,sha256:\$sha}"; done' | jq -s .)
