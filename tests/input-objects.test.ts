...
class Bucket {
 ...
 return { partNumber, etag: await sha256(bytes) };
 ...
 return upload;
  ...
  async head(key: string) { const blob = this.objects.get(key);

 return blob ? { size: blob.size } : null; }
  async get(key: string) { const blob = this.objects.get(key);

 return blob ? { size: blob.size, body: blob.stream(), arrayBuffer: () => blob.arrayBuffer() } : null; }
}
