...
<MaintainerShell active="queue" user={data.user || null}>
  <section class="maintainer-page" aria-labelledby="run-title">
    ...
    <section class="workbench-panel" aria-labelledby="attempts-title">
      ...
      {#each data.attempts as attempt}
        <article>
          ...
          <p>Revision: <code>{attempt.candidateRevisionId ?? 'No candidate produced'}</code></p>
    ...
    <section class="workbench-panel" aria-labelledby="dossiers-title">
      ...
      <ul>{#each data.dossiers as dossier}<li><a href={`/maintain/dossiers/${encodeURIComponent(dossier.id)}`}>{dossier.revision_id}</a></li>{:else}<li>No snapshot saved. Open the request to save its current evidence.</li>{/each}</ul>
    </section>
