...
function resolveVendorLinkTarget(path: string, target: string): string {
  if (typeof target !== 'string' || target.length === 0 || target.length > MAX_VENDOR_PATH_BYTES || [...target].some((character) => character <= '\u001f' || character === '\u007f' || character === '\\')) {
...
export function parseVendorArtifactManifest(raw: string, entriesRaw?: string, root = '/workspace'): VendorArtifactManifest {
  ...
  for (const [key, item] of Object.entries(metadataValue as Record<string, unknown>)) {
    if (!/^[A-Za-z][A-Za-z0-9_]{0,63}$/.test(key) || typeof item !== 'string' || item.length > 512 || item.includes('\u0000') || item.includes('\r') || item.includes('\n')) throw new Error('vendor metadata field is invalid');
    metadata[key] = item;
  }
