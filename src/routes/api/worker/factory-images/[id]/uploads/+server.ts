...
import { startFactoryImageUpload } from '$lib/server/factory-image-run';
...
export const POST: RequestHandler = async (event) => {
  try {
    ...
    return json(await startFactoryImageUpload(env, auth.worker, event.params.id, parseJsonRequest(body)), { headers: { 'Cache-Control': 'private, no-store' } });
