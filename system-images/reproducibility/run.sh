...
usage() {
  cat <<'EOF'
usage: system-images/reproducibility/run.sh [--oci|--boot|--all|--filesystem-fixture] [--gap SECONDS] [--output DIR]
...
boot_prerequisites() {
  ...
  [[ "$(uname -m)" == x86_64 ]] || missing+=(native-x86_64-builder)
  [[ "$(uname -m)" == "$(jq -er '.architecture' "$boot_profile" 2>/dev/null || true)" ]] || missing+=(native-profile-architecture)
...
filesystem_fixture_acceptance() {
  ...
  "$runtime" run --rm --network none "$builder_image" bash -lc 'command -v qemu-img && command -v sgdisk && command -v mkfs.fat' >/dev/null 2>&1 || { incomplete 'digest-pinned builder image lacks qemu-img, sgdisk, or mkfs.fat'; return 3; }
...
fetch_real_packages() {
  ...
  "$runtime" run --rm --network bridge -v "$repo_root:/repo:ro" -v "$fetch_output:/out:rw" "$builder_image" /repo/system-images/reproducibility/fetch-real-packages.sh /out
}
