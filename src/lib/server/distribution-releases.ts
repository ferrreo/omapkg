...
import { ownedRepositoryReleaseRepositories, type OwnedRepositoryPreparation } from './owned-repository';
...
function text(value: unknown, label: string, maximum = MAX_TEXT): string {
  if (typeof value !== 'string' || !value.trim() || value.length > maximum || [...value].some((character) => character <= '\u001f' || character === '\u007f')) reject(`${label} is invalid.`, 400);
...
function releaseManifestKey(kind: DistributionReleaseKind, channel: DistributionReleaseChannel, releaseId: string): string {
  ...
  switch (kind) {
    case 'system': return `distribution/releases/system/${channel}/${encoded}/manifest.json`;
    case 'opr': return `distribution/releases/opr/${channel}/${encoded}/manifest.json`;
    case 'resolved-transaction': return `distribution/releases/transactions/${channel}/${encoded}/manifest.json`;
  }
}
...
function publicManifestUrl(env: Env, kind: DistributionReleaseKind, channel: DistributionReleaseChannel, releaseId: string): string {
  ...
  switch (kind) {
    case 'system': return `${origin}/repo/releases/${channel}/${encoded}/manifest.json`;
    case 'opr': return `${origin}/repo/opr/${channel}/${encoded}/manifest.json`;
    case 'resolved-transaction': return `${origin}/repo/transactions/${channel}/${encoded}/manifest.json`;
  }
}
...
export async function prepareStoredRelease(env: Env, actor: Actor, preparation: OwnedRepositoryPreparation, channel: DistributionReleaseChannel, systemManifestDigest: string | null) {
  humanMaintainer(actor);
  const owned = await assertOwnedPackageChunks(env, preparation.packageChunks, preparation.lane, preparation.releaseId);
  const changes = [];
  for (const ref of owned.changelogDigests) {
    const row = await env.DB.prepare('SELECT document_json,markdown FROM cohort_changelogs WHERE cohort_id=? AND revision=? AND digest=?')
      .bind(ref.cohortId, ref.revision, ref.digest).first<{ document_json: string; markdown: string }>();
    if (!row || await sha256(row.document_json) !== ref.digest) reject('Release notes changed. Refresh and review the changes again.');
    changes.push({ cohortId: ref.cohortId, markdown: row.markdown });
  }
  const bytes = new TextEncoder().encode(canonicalJson({ schemaVersion: 1, cohorts: owned.changelogDigests, changes }));
  if (bytes.byteLength > 1_048_576) reject('Release notes are too large. Prepare a smaller release.');
  const hash = await sha256(bytes);
  const key = `private/distribution/changelogs/${hash}.json`;
  await immutableBytes(env, key, bytes, hash, 'application/json');
  const sourceRefs: Array<{ name: string; ref: string; digest: string }> = [];
  if (owned.omarchyPair) {
    const packages = await packageChunkRows(env, preparation.packageChunks);
    for (const name of owned.omarchyPair.packageNames) {
      const item = packages.find((value) => value.name === name);
      const revision = await env.DB.prepare('SELECT r.manifest_sha256 FROM owned_repository_artifacts a JOIN revisions r ON r.id=a.revision_id WHERE a.id=?')
        .bind(item?.releaseId).first<{ manifest_sha256: string }>();
      if (!revision) reject(`Reviewed source information for ${name} is unavailable.`);
      sourceRefs.push({ name, ref: owned.omarchyPair.commit, digest: revision.manifest_sha256 });
    }
  }
  return prepareDistributionRelease(env, actor, {
    kind: preparation.lane, channel, releaseId: preparation.releaseId, architectures: ['x86_64', 'aarch64'],
    repositories: ownedRepositoryReleaseRepositories(preparation), packageChunks: preparation.packageChunks, packageCount: preparation.packageCount,
    compatibility: { systemManifestDigest }, sourceRefs,
    changelog: { key, sha256: hash, size: bytes.byteLength, url: publicReleaseObjectUrl(env, 'changelog', hash) },
  });
}
...
export async function assertOmarchyPair(env: Env, rows: Array<{ item: Record<string, unknown>; row: OwnedPackageRow }>, candidateReleaseId: string): Promise<OmarchyPairBinding> {
  ...
  if (/^[0-9]+\.[0-9]+\.[0-9]+$/.test(candidateReleaseId)) {
    ...
    for (const chunk of rcChunks) {
      ...
      const object = await env.ARTIFACTS.get(chunk.object_key);
...
async function assertCandidateGates(env: Env, input: DistributionCandidateInput, kind: CandidateKind): Promise<void> {
  ...
  for (const cohortId of cohortIds) {
    ...
    const requiredOperations = ['install', 'upgrade', 'recovery', ...(kind === 'system' ? ['boot'] : [])];
    ...
    for (const operation of requiredOperations) {
        const operationName = operation as 'install' | 'upgrade' | 'recovery' | 'boot';
...
export async function signDistributionRelease(env: Env, actor: Actor | null, candidateId: string): Promise<{ candidateId: string; manifestSha256: string; signatureKey: string; signatureSha256: string }> {
  ...
  const signed = await finalizeManifest(env, row);
...
async function finalizeManifest(env: Env, row: CandidateRow): Promise<CandidateRow> {
  const manifest = JSON.parse(row.manifest_json) as ReleaseManifest;
...
export async function activateDistributionRelease(env: Env, actor: Actor | null, candidateId: string, expectedParent?: { digest: string | null; sequence: number | null }): Promise<{ candidateId: string; manifestSha256: string; transactionManifestSha256: string; sequence: number }> {
  ...
  const signedRow = await finalizeManifest(env, row);
  const signedOpr = activatingOprCounterpart ? await finalizeManifest(env, oprRow) : oprRow;
  ...
  const signedTransaction = await finalizeManifest(env, transaction.row);
  const timestamp = now();
...
export async function renewResolvedTransaction(env: Env, actor: Actor | null, channel: SystemReleaseChannel = 'stable', expectedParent?: { digest: string | null; sequence: number | null }): Promise<{ candidateId: string; manifestSha256: string; sequence: number }> {
  ...
  const signed = await finalizeManifest(env, transaction.row);
  const timestamp = now();
