...
<MaintainerShell active="cohorts" user={data.user}>
  <section class="maintainer-page">
    ...
    {:else if data.tab === 'tests'}
      ...
      <section class="workbench-panel"><h2>Native qualification evidence</h2><p>Plans bind exact candidate, selected input, artifact or final owned-universe root, profile, architecture and operation. Workers fetch reviewed plans and submit signed observations; this page never runs a test.</p>
        {#if data.qualificationEvidence.length}<div class="data-table-wrap"><table class="data-table"><thead><tr><th>Operation</th><th>Target</th><th>Coverage</th><th>Status</th><th>Worker</th><th>Recorded</th></tr></thead><tbody>{#each data.qualificationEvidence as evidence}<tr><td>{evidence.operation}</td><td>{evidence.architecture}</td><td>{evidence.coverage_kind === 'system' ? `Final universe · ${evidence.coverage_root_sha256}` : evidence.coverage_pkgbase}</td><td>{evidence.status}{#if evidence.reproducibility_status} · {evidence.reproducibility_status}{/if}{#if evidence.exception_count} · exception active{/if}</td><td>{evidence.worker_id}</td><td>{date(evidence.created_at)}</td></tr>{/each}</tbody></table></div>{:else}<p>No signed qualification evidence has been recorded for this revision.</p>{/if}
      </section>
      {#if data.canEdit}<section class="workbench-panel"><h2>Create reviewed qualification plan</h2><p>Paste exact plan JSON from the maintainer workflow. Include <code>coverage.kind</code> as <code>system</code> with the prepared owned-universe root for final system tests, or <code>member</code> for one package/reproduction output set.</p><form method="POST" action="?/qualificationPlan" class="review-form" use:enhance={enhanceAction}><div class="field"><label for="qualification-plan-json">Plan JSON</label><textarea id="qualification-plan-json" name="planJson" rows="12" maxlength="1048576" required bind:value={planDraft} placeholder="Paste reviewed plan JSON"></textarea></div><button class="button button--primary" type="submit" disabled={busy}>Create immutable plan</button></form>
        {#if data.qualificationPlans.length}<div class="data-table-wrap"><table class="data-table"><thead><tr><th>Plan</th><th>Operation</th><th>Coverage/root/release</th><th>Profile</th><th>Reviews</th><th>Digest</th><th>Actions</th></tr></thead><tbody>{#each data.qualificationPlans as plan}<tr><td><code>{plan.id}</code><br /><span class="timestamp">{date(plan.created_at)}</span></td><td>{plan.operation} · {plan.architecture}</td><td>{plan.coverage_kind === 'system' ? `system root ${plan.coverage_root_sha256} · ${plan.coverage_release_id}` : plan.coverage_pkgbase}</td><td>{planField(plan.plan_json, 'profile.id') || 'profile not named'}<br /><span class="timestamp">{planField(plan.plan_json, 'profile.sha256') || 'profile digest missing'}</span></td><td>{plan.review_kinds}/2 kinds<br /><span class="timestamp">{plan.review_actors}/2 reviewers</span></td><td class="hash">{plan.plan_sha256}</td><td><details><summary>Review</summary><form method="POST" action="?/qualificationReview" class="review-form" use:enhance={enhanceAction}><input type="hidden" name="planId" value={plan.id} /><div class="field"><label for={`qualification-kind-${plan.id}`}>Review kind</label><select id={`qualification-kind-${plan.id}`} name="kind"><option value="area">Area</option><option value="security">Security</option></select></div><div class="field"><label for={`qualification-reason-${plan.id}`}>Reason</label><input id={`qualification-reason-${plan.id}`} name="reason" required maxlength="2000" /></div><button class="button" type="submit" disabled={busy}>Record review</button></form><details><summary>Exact plan JSON</summary><pre class="code-block">{plan.plan_json}</pre></details></details></td></tr>{/each}</tbody></table></div>{/if}
        <p>Worker command: <code>opr-worker qualification --origin https://… --plan-id PLAN_ID --config /var/lib/opr-worker/config.json --output /var/lib/opr-worker/qualification.json --submit</code>. <a href="/docs/reproducibility">Read worker qualification and reproducibility guidance</a>.</p>
      </section>{/if}
    {:else}
