...
export const GET: RequestHandler = async ({ platform, params }) => {
  ...
  const lane = params.lane === 'releases' ? 'system' : params.lane === 'opr' ? 'opr' : null;
  const releaseId = params.releaseId ?? '';
  const collection = params.collection ?? '';
  const architecture = params.architecture as Architecture;
  const filename = params.filename ?? '';
  if (!lane || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(releaseId) || !collections.has(collection) || !['x86_64', 'aarch64'].includes(architecture) || !filenamePattern.test(filename) || filename.includes('/')) error(404, 'Object not found.');
