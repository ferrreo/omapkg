...
export function parseRecipeSourceBundle(input: unknown): RecipeSourceBundle {
  ...
  for (const source of value.sources) {
    ...
    for (const url of source.kind === 'file' ? source.redirects : []) {
      ...
      if (parsed.username || parsed.password || hasControlOrSpace(url) || url.includes('\\')) throw new Error('Unsafe source redirect URL');
    }
  }
...
export function recipeSources(capture: RecipeCapture, metadata: Srcinfo, architecture: Architecture): Pick<RecipeSourcePlan, 'sources' | 'validpgpkeys'> {
  ...
  for (const suffix of ['', `_${architecture}`]) {
    ...
    for (const [key, values] of sums) {
      ...
      if (length === undefined || values.length !== raw.length || values.some((value) => value !== 'SKIP' && !new RegExp(`^[a-fA-F0-9]{${length}}$`).test(value))) throw new Error(`Invalid or unsupported source checksum array: ${key}`);
    }
    ...
    for (let index = 0; index < raw.length; index++) {
      ...
      if (!git && capture.files.some((file) => file.path === name)) sources.push({ ...common, kind: 'local', path: name });
        ...
        if (fragment && (!match || hasControlOrSpace(match[2]) || /[~^:?*=[\\]|\.\.|@\{|\/$|\.$|\.lock(?:\/|$)|\/\/|(?:^|\/)\./.test(match[2]) || match[2].startsWith('-') || match[2].startsWith('/'))) throw new Error('Invalid Git source reference');

        if (match?.[1] === 'commit' && !/^[a-fA-F0-9]{40}$/.test(match[2])) throw new Error('Git commit sources require a full immutable commit');
        const ref = !match ? { kind: 'head' as const, value: 'HEAD' } : match[1] === 'commit' ? { kind: 'commit' as const, value: match[2] } : match[1] === 'tag' ? { kind: 'tag' as const, value: match[2] } : { kind: 'branch' as const, value: match[2] };
        sources.push({ ...common, kind: 'git', url, ref, signed: query === 'signed' });
