ALTER TABLE recipe_inspections ADD COLUMN recipe_override_sha256 TEXT REFERENCES input_objects(sha256);
ALTER TABLE recipe_inspections ADD COLUMN factory_run_id TEXT REFERENCES factory_runs(id);
ALTER TABLE recipe_inspections ADD COLUMN factory_attempt INTEGER;

CREATE INDEX recipe_inspection_factory ON recipe_inspections(factory_run_id,factory_attempt);

DROP TRIGGER recipe_inspection_scope;
CREATE TRIGGER recipe_inspection_scope BEFORE UPDATE OF capture_sha256,architecture,image_id,image_ref,catalog_revision,catalog_sha256,requested_by,reason,created_at,recipe_override_sha256,factory_run_id,factory_attempt ON recipe_inspections
...
CREATE VIEW current_recipe_inspections AS
 ...
 AND (i.factory_run_id IS NULL OR EXISTS(
   ...
   WHERE run.id=i.factory_run_id AND run.execution_scope='private' AND (
     ...
     (run.status='queued' AND run.attempt_count=i.factory_attempt-1 AND run.current_attempt=i.factory_attempt-1 AND run.lease_token IS NULL AND run.lease_expires_at IS NULL))))
