...
export async function validateRecipePolicy(revision: Revision): Promise<void> {
  const policy = readOprEvidence(JSON.parse(revision.sbom_json))?.recipePolicy as {
    ...
    vendorArtifact?: FactoryCandidate['vendorArtifact'];
  } | undefined;
  ...
  const candidate: FactoryCandidate = {
    ...
    recipeMode: 'template', template: policy.template, buildCommands: [], packageCommands: [], smokeCommands: [], vendorArtifact: policy.vendorArtifact,
  };
