...
export function parseFrozenManifest(value: unknown): FrozenManifest {
  ...
  if ((helperAnalysis && m.shellAnalysis !== 'helper') || m.schemaVersion !== 1 || !['bootstrap', 'owned'].includes(m.purpose) || !['x86_64', 'aarch64'].includes(m.architecture) ||
      ...
      typeof m.helperImage !== 'string' || m.helperImage.length > 1024 || hasControlOrSpace(m.helperImage) || /\s/.test(m.helperImage) || !/^.+@sha256:[a-f0-9]{64}$/.test(m.helperImage) ||
...
export function assertNativeHost(value: unknown, architecture: Architecture): FrozenEvidence['host'] {
  ...
  if (host.architecture !== architecture || !INPUT_HASH.test(host.cpuInfoSha256) || !['podman', 'docker'].includes(host.runtime) ||
      [host.kernel, host.cpuModel, host.runtimeVersion, host.goVersion].some((value) => typeof value !== 'string' || !value || value.length > 4096 || [...value].some((character) => character <= '\u001f' || character === '\u007f'))) {
