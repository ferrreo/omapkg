<script lang="ts">
  ...
  type DescribedRevision = Revision & { recipeUrl?: string | null; description?: string | null; recipePolicy?: { mode: string; recorded: boolean }; runtimeExceptions?: Array<{ findingSha256: string; reason: string }> };
  ...
  onMount(() => startVisibleRefresh(
    () => ['generating', 'queued', 'building', 'blocked'].includes(request?.status || ''),
    () => { void invalidateAll(); }
  ));
</script>
      ...
      {#if request.status === 'blocked'}
        <section class="workbench-panel" aria-labelledby="dependency-blockers-title">
          ...
          <div class="release-actions">
            ...
            <form method="POST" action="?/regenerate"><label for="dependency-regenerate">Recipe or exception changes needed</label><input id="dependency-regenerate" name="reason" required maxlength="2000" /><button class="button" type="submit">Regenerate for review</button></form>
          </div>
        </section>
      {/if}
