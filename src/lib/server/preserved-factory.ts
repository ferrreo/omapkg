...
export async function deriveFactoryInputLocks(env: Pick<FactoryEnv, 'DB' | 'ARTIFACTS'>, sourceRevisionId: string, targetRevisionId: string, targetRecipeSha256: string): Promise<string[]> {
  ...
  for (const row of rows.results) {
    ...
    await env.DB.batch([
      ...
      env.DB.prepare('INSERT OR IGNORE INTO input_lock_objects(lock_sha256,object_sha256) SELECT ?,CASE WHEN object_sha256=? THEN ? ELSE object_sha256 END FROM input_lock_objects WHERE lock_sha256=?').bind(lockSha, row.lock_sha256, lockSha, row.lock_sha256),
