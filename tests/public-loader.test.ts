...
const schema = `
CREATE TABLE requests(id TEXT PRIMARY KEY,name TEXT,upstream_url TEXT,status TEXT NOT NULL);
CREATE TABLE revisions(id TEXT PRIMARY KEY,request_id TEXT NOT NULL,description TEXT,recipe TEXT,explanation TEXT,sources_json TEXT,license TEXT);
...
test('public loader honors channel, surface, and architecture filters and returns final descriptions', async () => {
  ...
  try {
    ...
    db.prepare('INSERT INTO revisions(id,request_id,description,recipe,explanation,sources_json,license) VALUES(?,?,?,?,?,?,?), (?,?,?,?,?,?,?), (?,?,?,?,?,?,?)').bind(
      'v-stable', 'q1', 'Stable description', "pkgdesc='Ignored legacy text'", 'audit detail', '[]', 'MIT',
      'v-dev', 'q1', 'Dev description', "pkgdesc='Dev legacy text'", 'audit detail', '[]', 'MIT',
      'v-old', 'q1', null, "pkgdesc='Legacy parsed text'", 'audit detail', '[]', 'MIT',
    ).run();
