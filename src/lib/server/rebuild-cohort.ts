...
function string(input: unknown, label: string, max = 4096): string {
  if (typeof input !== 'string' || !input || input.length > max || [...input].some((character) => character <= '\u001f' || character === '\u007f')) throw new PolicyError(400, `Rebuild report has an invalid ${label}.`);
...
export async function previewRebuildCohort(db: D1Database, cohortId: string, input: unknown): Promise<RebuildScopePreview> {
  ...
  return { reportSha256, cohortId, expectedRevision: cohort.current_revision, title: manifest.title, lane: manifest.lane, systemVersion: manifest.systemVersion,
    ...
    targetPlans: requiredArchitectures.map((architecture) => { const plan = report.plans.find((item) => item.architecture === architecture);
...
export async function rebuildCoverage(db: D1Database, at = now()): Promise<RebuildCoverage> {
  ...
  const queue = requiredArchitectures.map((architecture) => { const row = queueRows.find((item) => item.architecture === architecture);
 ...
 return { architecture, expected, succeeded: Number(row?.succeeded ?? 0), measured: expected > 0 };
  }));
