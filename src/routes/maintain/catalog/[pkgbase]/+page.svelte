...
<MaintainerShell active="catalog" user={data.user}>
  <section class="maintainer-page">
    ...
    <section class="workbench-panel"><div class="workbench-panel__head"><h2>Revision {data.record.revision}</h2><span class="tag">{data.manifest.lane === 'system' ? 'Versioned system release' : 'Independent OPR package'}</span></div><dl class="detail-list"><div class="detail-list__row"><dt>Repository / role</dt><dd>{data.manifest.collection} / {data.manifest.role}</dd></div><div class="detail-list__row"><dt>Owner</dt><dd>{data.manifest.ownerArea}</dd></div><div class="detail-list__row"><dt>Source</dt><dd><a href={data.manifest.upstreamUrl}>{data.manifest.upstreamUrl}</a></dd></div><div class="detail-list__row"><dt>Build targets</dt><dd>{data.manifest.architectures.join(', ')}</dd></div><div class="detail-list__row"><dt>Package outputs</dt><dd>{data.manifest.outputs.join(', ')} · {data.manifest.artifactArchitecture}</dd></div></dl>{#each data.manifest.architectureExceptions as exception}<p class="form-notice">{exception.architecture} exception: {exception.reason}</p>{/each}<details><summary>Exact policy evidence</summary><pre class="code-block">{JSON.stringify(data.manifest, null, 2)}</pre><p class="hash">{data.record.manifest_sha256}</p></details></section>
    <section class="workbench-panel"><h2>Independent ownership review</h2><p>Area and security reviewers approve this exact source, release policy and architecture scope. This does not approve a recipe or publish a binary.</p>{#each ['area', 'security'] as kind}{@const review = data.reviews.find((item) => item.kind === kind)}<p><strong>{kind === 'area' ? 'Area review' : 'Security review'}:</strong> {review ? review.reason : 'Required'}</p>{/each}
