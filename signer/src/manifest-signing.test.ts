...
test('signs an exact distribution manifest through the shared signer contract', async () => {
 ...
 return bytes ? { size: bytes.byteLength, customMetadata: metadata.get(key) ?? {} } : null; },
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === '/api/internal/signing-events') { events.push(JSON.parse(String(init?.body ?? '{}')));
...
test('signs a native schema v2 core database with reviewed output context', async () => {
  ...
  const reproducibilityFiles = [{ filename: 'demo-1.0-1-x86_64.pkg.tar.zst', size: 1, sha256: packageDigest }];
  ...
  const provenance = JSON.stringify({ schemaVersion: 2, attempt: 1, outputContract, buildId: 'build-db', revisionId: 'revision-db', workerId: 'worker-db',
    ...
    startedAt: '2026-09-10T00:00:00.000Z', finishedAt: '2026-09-10T00:01:00.000Z', buildEnvironment,
    reproducibility: { schemaVersion: 1, status: 'reproducibility-contract-verified', mode: 'single-build', target: 'x86_64',
      inputs: { recipeSha256: '2'.repeat(64), sourceManifestSha256: await sha256(new TextEncoder().encode(canonicalJson([]))), inputLockSha256: '', dependencyPlanSha256: '', imageDigest: `sha256:${'c'.repeat(64)}`, sourceDateEpoch: 1 },
      ...
      outputs: { setSha256: await sha256(new TextEncoder().encode(canonicalJson(reproducibilityFiles))), files: reproducibilityFiles, unexpected: [], prohibitedPaths: [] }, limitations: ['single execution does not establish independent byte reproduction'] },
    runtimeTests: [{ outputs: ['demo'], environment: runtimeEnvironment, smokePassed: true,
