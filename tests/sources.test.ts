...
function requestEvent(db: TestD1, reads: string[], request = new Request(source)) {
  return {
    ...
    platform: { env: { DB: asD1(db), PUBLIC_ORIGIN: origin, ARTIFACTS: {
      async get(key: string) { reads.push(key);

 return { body: new Blob(['source']).stream(), size: 6, httpEtag: '"test"' }; }
    } } }
...
async function normalClaimFixture() {
  ...
  db.prepare('INSERT INTO workers(id,name,architecture,public_key,status,enrolled_at,last_seen_at,capabilities_json) VALUES(?,?,?,?,?,?,?,?)')
    .bind('source-worker', 'source-worker', 'x86_64', encodedPublicKey, 'active', timestamp, timestamp, JSON.stringify(['single-build-reproducibility-v1'])).run();
