  ...
  <section class="maintainer-page">
    ...
    {#if data.tab === 'overview'}
      <section class="workbench-panel"><h2>Why these packages update together</h2><p>Scope includes exact ownership policy and recipe inputs. Adding, removing or rebinding a member creates a new revision and restarts phase review.</p>
        {#each data.manifest.members as member}
          <article class="member"><div class="member-head"><h3><a href={`/maintain/catalog/${encodeURIComponent(member.pkgbase)}`}>{member.pkgbase}</a></h3><span class="tag">{member.policy.collection} · {member.policy.ownerArea}</span></div><p>{member.reason} <span class="timestamp">({member.cause.replaceAll('-', ' ')})</span></p>
            ...
            {#if editable}<details><summary>Update this member</summary>
      ...
      {#if editable}<section class="workbench-panel"><h2>Add an affected package</h2><form method="GET" class="search-form"><input type="hidden" name="tab" value="overview" /><div class="field"><label for="add-search">Search catalog identities</label><input id="add-search" name="search" value={data.search} maxlength="100" /></div><button class="button" type="submit">Search</button></form>
        {#if chosen}<form method="POST" action="?/scope" class="review-form" use:enhance={enhanceAction}><input type="hidden" name="revision" value={data.record.current_revision} /><input type="hidden" name="digest" value={data.record.manifest_sha256} /><input type="hidden" name="action" value="add" /><input type="hidden" name="catalogRevision" value={chosen.revision} />
          <div class="field"><label for="add-package">Affected package</label><select id="add-package" name="pkgbase" bind:value={addition}>{#each data.packages as candidate}<option value={candidate.pkgbase} disabled={data.manifest.members.some((member) => member.pkgbase === candidate.pkgbase)}>{candidate.pkgbase} · {candidate.collection}</option>{/each}</select></div>
          <div class="field"><label for="add-cause">Cause</label><select id="add-cause" name="cause">{#each cohortCauses as cause}<option value={cause}>{cause.replaceAll('-', ' ')}</option>{/each}</select></div><div class="field"><label for="add-reason">Why does it need to update with this cohort?</label><textarea id="add-reason" name="reason" required maxlength="2000" rows="2"></textarea></div><button class="button" type="submit" disabled={busy || data.manifest.members.some((member) => member.pkgbase === addition)}>Propose added member</button>
    ...
    {:else if data.tab === 'tests'}
      <section class="workbench-panel"><h2>Native target matrix</h2><p>Required failures and missing targets block progression. A reviewed hardware exception stays visible beside its target.</p>{#each data.gate.matrix as row}<article class="matrix-row"><strong>{row.pkgbase}</strong><span>{row.architecture}</span><span>{row.required ? row.status : 'Not required'}</span>{#if row.buildId}<a href={`/maintain/builds/${encodeURIComponent(row.buildId)}`}>Build evidence</a>{/if}{#if row.reason}<p>{row.reason}</p>{/if}</article>{/each}</section>
      <section class="workbench-panel"><h2>Required checks</h2>{#if data.gate.blockers.length}<ul class="blocker-list">{#each data.gate.blockers as blocker}<li><strong>{blocker.pkgbase ?? 'Cohort'}{blocker.architecture ? ` · ${blocker.architecture}` : ''}:</strong> {blocker.reason}{#if blocker.href} <a href={blocker.href}>Open evidence</a>{/if}</li>{/each}</ul>{:else}<p>Checks for the next phase are ready. Later native, compatibility and release gates still apply.</p>{/if}</section>
    {:else}
